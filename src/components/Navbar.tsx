"use client";

import Link from 'next/link';
import Image from 'next/image';
import { usePathname } from 'next/navigation';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';
import { useState, useEffect } from 'react';
import { PlaceHolderImages } from '@/lib/placeholder-images';

export function Navbar() {
  const [isScrolled, setIsScrolled] = useState(false);
  const pathname = usePathname();
  const logo = PlaceHolderImages.find(img => img.id === 'logo-aziendale');

  // Determina se la pagina corrente ha un elemento Hero scuro che richiede testo bianco
  const isDarkHeroPage = pathname === '/' || pathname.startsWith('/servizi/');

  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 20);
    };
    window.addEventListener('scroll', handleScroll);
    return () => window.removeEventListener('scroll', handleScroll);
  }, []);

  // La navbar è "trasparente" solo se siamo su una pagina con Hero scuro E non abbiamo scrollato
  const isTransparent = isDarkHeroPage && !isScrolled;

  return (
    <nav className={cn(
      "fixed top-0 left-0 right-0 z-50 transition-all duration-300 px-6",
      isTransparent ? "bg-transparent py-4" : "bg-white/95 backdrop-blur-md border-b shadow-lg py-2"
    )}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 group">
          <div className="relative h-10 w-10 md:h-12 md:w-12 rounded-xl overflow-hidden shadow-md group-hover:scale-110 transition-transform bg-white">
            {logo && (
              <Image 
                src={logo.imageUrl} 
                alt="LI-COSTRUZIONI Logo" 
                fill 
                className="object-contain p-1.5"
                data-ai-hint={logo.imageHint}
              />
            )}
          </div>
          <span className={cn(
            "font-headline font-extrabold text-lg md:text-xl tracking-tighter transition-colors",
            isTransparent ? "text-white" : "text-primary"
          )}>
            LI-COSTRUZIONI
          </span>
        </Link>

        <div className={cn(
          "hidden md:flex items-center gap-8 font-semibold",
          isTransparent ? "text-white/90" : "text-primary/90"
        )}>
          <Link href="/" className="text-sm hover:text-accent transition-colors">Home</Link>
          <Link href="/servizi" className="text-sm hover:text-accent transition-colors">Servizi</Link>
          <Link href="/progetti" className="text-sm hover:text-accent transition-colors">Progetti</Link>
          <Link href="/chi-siamo" className="text-sm hover:text-accent transition-colors">Chi Siamo</Link>
          <Link href="/contatti" className="text-sm hover:text-accent transition-colors">Contatti</Link>
        </div>

        <Button asChild className="hidden sm:flex font-bold shadow-lg transition-all" variant={isTransparent ? "secondary" : "default"}>
          <Link href="/contatti">Richiedi Preventivo</Link>
        </Button>
      </div>
    </nav>
  );
}