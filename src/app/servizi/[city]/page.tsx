"use client";

import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import {
  CheckCircle2,
  MapPin,
  FileText,
  Smartphone,
  ArrowRight,
  Building2,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import Link from "next/link";
import React from "react";

// --- DATI DELLE LOCALITÀ ---
const cityContent: Record<string, { desc: string; focus: string; name: string }> = {
  terracina: {
    name: "Impresa Edile Terracina",
    desc: "Esperti in ristrutturazioni fronte mare e consolidamento edifici nel centro storico.",
    focus: "Manutenzioni Marine & Centro Storico",
  },
  roma: {
    name: "Cantieri a Roma",
    desc: "Specializzati in ristrutturazioni di appartamenti di pregio e gestione cantieri in zone ZTL.",
    focus: "Ristrutturazioni Luxury & Condomini",
  },
  latina: {
    name: "Ristrutturazioni Latina",
    desc: "Punto di riferimento per nuove costruzioni residenziali e adeguamento energetico.",
    focus: "Efficienza Energetica & Nuove Cubature",
  },
  sabaudia: {
    name: "Edilizia Sabaudia",
    desc: "Interventi su ville e architettura razionalista nel rispetto dei vincoli paesaggistici.",
    focus: "Ville d'Elite & Vincoli Ambientali",
  },
  "san-felice-circeo": {
    name: "San Felice Circeo",
    desc: "Eccellenza nella posa di materiali naturali e residenze estive di alto profilo.",
    focus: "Posa Materiali Pregiati & Design Estivo",
  },
};

const formatCityName = (slug: string): string =>
  slug
    .split("-")
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(" ");

export default function LocalServicePage({ params }: { params: { city: string } }) {
  const citySlug = params.city;
  const cityKey = cityContent[citySlug] ? citySlug : "terracina";
  const city = formatCityName(cityKey);
  const specificContent = cityContent[cityKey];

  return (
    <article className="min-h-screen bg-white">
      {/* Hero Section */}
      <section className="relative pt-40 pb-24 px-6 bg-slate-100 overflow-hidden border-b border-slate-200">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-amber-500/10 via-transparent to-transparent opacity-100" />
        
        <div className="max-w-6xl mx-auto relative z-10 text-center md:text-left">
          <Badge className="bg-amber-500 text-slate-950 mb-6 font-black uppercase tracking-widest px-4 py-1.5 text-[10px] rounded-full shadow-sm border-none">
            📍 {specificContent.name}
          </Badge>

          <h1 className="text-4xl md:text-7xl font-black text-slate-900 mb-6 tracking-tighter uppercase leading-[0.95]">
            Costruiamo il <span className="text-amber-500">Valore</span>{" "}
            <br className="hidden md:block" />
            del tuo immobile a {city}
          </h1>

          <p className="text-lg md:text-xl text-slate-600 mb-10 max-w-2xl leading-relaxed font-medium">
            {specificContent.desc}
          </p>

          <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <Button size="lg" className="bg-slate-900 text-white hover:bg-amber-500 hover:text-slate-950 font-black h-14 px-10 rounded-2xl uppercase text-xs tracking-widest" asChild>
              <Link href="/contatti">
                Richiedi Preventivo <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
          </div>
        </div>
      </section>

      {/* Trust Section */}
      <section className="py-24 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            <div className="lg:col-span-5">
              <span className="text-amber-600 font-black uppercase tracking-[0.3em] text-[10px] mb-4 block">Perché Sceglierci</span>
              <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-6 tracking-tighter uppercase">
                Impresa edile di <br /> riferimento a {city}
              </h2>
              <div className="space-y-3">
                {[`Focus: ${specificContent.focus}`, "Gestione pratiche CILA/SCIA", "Direzione tecnica costante", "Garanzia decennale"].map((item, i) => (
                  <div key={i} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <CheckCircle2 className="text-amber-500 h-5 w-5" />
                    <span className="font-bold text-slate-700 text-[11px] uppercase tracking-wider">{item}</span>
                  </div>
                ))}
              </div>
            </div>

            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { icon: MapPin, t: "Sopralluogo", d: `Analisi tecnica e rilievo metrico gratuito a ${city}.` },
                { icon: FileText, t: "Trasparenza", d: "Computo metrico estimativo analitico senza sorprese." },
                { icon: Building2, t: "Cantiere", d: "Sviluppo lavori con personale diretto e pulizia costante." },
                { icon: Smartphone, t: "Feedback", d: "Chat dedicata e report fotografici settimanali." },
              ].map((card, i) => (
                <Card key={i} className="border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 group rounded-[2rem] p-4">
                  <CardHeader className="pb-2">
                    <div className="h-12 w-12 rounded-xl bg-slate-900 flex items-center justify-center mb-4 group-hover:bg-amber-500 transition-colors">
                      <card.icon className="h-6 w-6 text-amber-500 group-hover:text-slate-900" />
                    </div>
                    <CardTitle className="text-lg font-black uppercase text-slate-900">{card.t}</CardTitle>
                  </CardHeader>
                  <CardContent className="text-slate-500 text-sm">{card.d}</CardContent>
                </Card>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* --- SEZIONE ALTRE LOCALITÀ (SEO) --- */}
      <section className="py-16 px-6 bg-slate-50 border-t border-slate-200">
        <div className="max-w-6xl mx-auto text-center md:text-left">
          <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter mb-8 italic">
            Operatività nel Lazio
          </h2>
          <div className="flex flex-wrap gap-3 justify-center md:justify-start">
            {Object.entries(cityContent).map(([slug, content]) => (
              <Link
                key={slug}
                href={`/servizi/${slug}`}
                className={`px-6 py-3 rounded-full border-2 font-bold text-[10px] uppercase tracking-widest transition-all ${
                  slug === citySlug
                    ? "border-amber-500 text-amber-600 bg-white shadow-sm"
                    : "border-slate-200 text-slate-500 hover:border-amber-500 hover:text-amber-500 bg-transparent"
                }`}
              >
                {content.name}
              </Link>
            ))}
          </div>
        </div>
      </section>

      {/* CTA Final Section */}
      <section className="py-24 px-6 bg-slate-200 relative">
        <div className="max-w-5xl mx-auto">
          <div className="bg-white rounded-[3rem] p-12 md:p-20 text-center shadow-2xl border-b-8 border-b-amber-500">
            <h2 className="text-3xl md:text-5xl font-black text-slate-950 mb-6 tracking-tighter uppercase">
              Iniziamo il tuo progetto <br /> a <span className="text-amber-500">{city}</span>
            </h2>
            <div className="flex flex-col sm:flex-row justify-center gap-4 mt-10">
              <Button size="lg" className="bg-slate-950 text-white hover:bg-amber-500 hover:text-slate-950 font-black h-16 px-12 rounded-2xl transition-all shadow-xl uppercase text-xs tracking-widest" asChild>
                <Link href="/contatti">Parla con Luca o Jessica</Link>
              </Button>
            </div>
          </div>
        </div>
      </section>
    </article>
  );
}